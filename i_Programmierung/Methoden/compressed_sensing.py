...
def random_sampling(N, X_ref, p, rauschen, rng_cs):
    ...
    Die Messmatrix A wird als Produkt von R und Psi_inv gebildet, wobei R eine zufällige Auswahl von Zeilen der Einheitsmatrix ist.
    """
    X_ref = als_zeitreihen_matrix(X_ref)
    k   = int(round(p * N))
    k   = max(1, min(k, N))
    idx = np.sort(rng_cs.choice(N, k, replace=False))
    R   = np.eye(N)[idx]
    A   = R @ Psi_inv

    sigma_liste   = np.zeros(N_KOMPONENTEN)
    epsilon_liste = np.zeros(N_KOMPONENTEN)
    y_liste = []
...
def delta_total_metrik(q1_rek, X_ref, C_ref, mu):
    """Fehlermetrik für q1 berechnen, die die Abweichung der Jacobi-Konstanten und die Abweichung der Trajektorie kombiniert."""
