...
from i_Programmierung.Konstanten import mu, Radius, L_fundamental, EIGENWERT_TOLERANZ, delta
...
def monodromy(t, y_kombiniert, mu):
    ...
    d_state = bewegungsgleichungen(t, state, mu)
    ...
    for vorzeichen in [1, -1]:
        ...
        for j in range (sol.y.shape[1]):
            ...
            for v_opt in ev_stabil:
                ...
                m0_pert = m0_orbit + vorzeichen * delta * v_lokal
...
trajektorie_optimal_sol = solve_ivp(bewegungsgleichungen,
    [t_opt, 0],
    state_at_topt,
    args=(mu,),
