...
for rauschen, farbe, label_r in vorhanden:
    ...
    ax_bahn.plot(rek['q1_rek'], q2_ref, color=farbe, lw=1.2, ls='--', alpha=0.9,
                 label=f'{label_r} ($\\alpha_{{\\min}}={rek["alpha_min_cs"]:.3f}$)')
ax_bahn.set_xlim(*grenzen_q1)
ax_bahn.set_xlabel(LABEL_Q1)
ax_bahn.set_ylabel(LABEL_Q2, fontsize=fontsize)
ax_bahn.set_title(r'Bahnprojektion bei $\alpha_{\min}^{\mathrm{CS}}$' + '\n', fontsize=fontsize)
...
save_diagram(fig_bahn, 'cs_trajektorienrekonstruktion_2d.pdf', unterordner = 'physik_rekonstruktion')
plt.show()

schwingung_ref = q1_ref - q1_ref.mean()
for rauschen, _farbe, label_r in vorhanden:
    rek = rekonstruktionen[rauschen]
    q1_rek = rek['q1_rek']
    korrelation = float(np.corrcoef(q1_rek - q1_rek.mean(), schwingung_ref)[0, 1])
