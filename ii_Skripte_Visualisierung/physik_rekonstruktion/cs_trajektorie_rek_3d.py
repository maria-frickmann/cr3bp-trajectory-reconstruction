...
from ii_Skripte_Visualisierung.setup_plot import (farben_rauschen, labels_rauschen,
                                                  ...
                                                  SYM_ALPHA_MIN_CS, LABEL_ALPHA_MIN_CS)
...
for rauschen, farbe, label_r in vorhanden:
    ...
    ax.plot(rek['q1_rek'], q2_ref, q3_ref, color=farbe, lw=1.3, ls='--',
            label=rf'{label_r} (${SYM_ALPHA_MIN_CS} = {rek["alpha_min_cs"]:.3f}$)', zorder=5)
    
dx = 0.03 * (xlim[1] - xlim[0])
ax.scatter(-mu, 0, 0, color=ERDE, s=60, zorder=12, depthshade=False)
ax.text(-mu - dx, 0, 0, 'Erde', fontsize=fontsize, color=ERDE, ha='right', va='center', zorder=13)
...
titel = ax.set_title(f'Trajektorienrekonstruktion bei {LABEL_ALPHA_MIN_CS}' + '\n',
...
save_diagram(fig, 'cs_trajektorienrekonstruktion_3d.pdf', unterordner='physik_rekonstruktion',
             oben_bis=titel)
plt.show()