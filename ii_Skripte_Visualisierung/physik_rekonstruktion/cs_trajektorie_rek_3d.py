...
X_ref            = meta['X_ref']                       # (N, 6): q1, q2, q3, p1, p2, p3
...
for rauschen, farbe, label_r in vorhanden:
    ...
    ax.plot(rek['q1_rek'], q2_ref, q3_ref, color=farbe, lw=1.3, ls='--',
            label=f'{label_r} ($\\alpha_{{\\min}}={rek["alpha_min_cs"]:.3f}$)', zorder=5)

Axenbeschriftung(ax)
ax.set_title(r'Trajektorienrekonstruktion bei $\alpha_{\min}^{\mathrm{CS}}$' + '\n',
             fontsize=fontsize)
ax.legend(frameon=False, fontsize=fontsize, loc='best')
fig.tight_layout()
save_diagram(fig, 'trajektorienrekonstruktion_3d.pdf', unterordner = 'physik_rekonstruktion')
plt.show()