...
from ii_Skripte_Visualisierung.setup_plot import (farben_rauschen, labels_rauschen,
                                                  ...
                                                  LABEL_KORR_CS, LABEL_MESSANTEIL,
                                                  LABEL_FORMKORRELATION, SYM_ALPHA_MIN_CS)
...
for rauschen, farbe, label_rauschen in zip(meta['rauschen_liste'],
    ...
    ax.plot(kurve['alpha'], kurve['korr_avg'], marker='o', markersize=4, linewidth=1.4,
            color=farbe,
            label=f'{LABEL_KORR_CS} Rauschen {label_rauschen}')
    if kurve['alpha_min_cs'] is not None:
        a_min = kurve['alpha_min_cs']
        ax.axvline(a_min, color=farbe, linestyle='-.', linewidth=1.2,
                   label=rf'${SYM_ALPHA_MIN_CS} = {a_min:.2f}$ ({label_rauschen})')

ax.set_xlabel(LABEL_MESSANTEIL, fontsize=fontsize)
ax.set_ylabel(LABEL_FORMKORRELATION, fontsize=fontsize)
ax.set_ylim(-0.15, 1.05)
ax.set_title('Formkorrelation der CS Rekonstruktion für verschiedene Rauschen', fontsize=fontsize)
...
fig.tight_layout()
save_diagram(fig, 'cs_verschiedene_rauschen_plot.pdf', unterordner='Kriterium_alpha_min')
plt.show()