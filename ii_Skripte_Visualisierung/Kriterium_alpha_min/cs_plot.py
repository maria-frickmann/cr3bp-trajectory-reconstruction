...
ax.axhline(rho, color='0.35', linestyle='dashed', linewidth=1.1,
           label=rf'$\rho = {rho:.2f}$ (Form-Schwelle)')
...
for rauschen, farbe, label_rauschen in zip(meta['rauschen_liste'], farben_rauschen, labels_rauschen):
    ...
    if kurve['alpha_min_cs'] is not None:
        ax.axvline(kurve['alpha_min_cs'], color=farbe, linestyle='-.', linewidth=1.2,
            ...
            else r'$\alpha_{\min}^{\mathrm{CS}}$ (Form-Kriterium)')
        alpha_min_beschriftet = True

ax.set_xlabel(r'Messanteil $\alpha$')
ax.set_ylabel(r'Formkorrelation', fontsize=fontsize)
ax.set_ylim(-0.15, 1.05)
ax.set_title(r'Formkorrelation der CS-Rekonstruktion für verschiedene Rauschen', fontsize=fontsize)
