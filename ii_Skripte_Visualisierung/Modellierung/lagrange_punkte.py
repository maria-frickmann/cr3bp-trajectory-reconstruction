...
for name, p in zip(['L1', 'L2', 'L3', 'L4', 'L5'], l_punkte):
    lx, ly = p[0], p[1]
    lz = V_mu(lx, ly, 0.0, mu) + 0.02
    ax.scatter([lx], [ly], [lz], color=WEISS, s=14, zorder=6)
    ax.plot([lx, lx], [ly, ly], [lz, lz + 0.28],
            color=WEISS, lw=1.0, zorder=6)
    ax.text(lx, ly, lz + 0.34, name, color=SCHWARZ, fontsize=fontsize + 1,
            ha="center", va="bottom", zorder=7,
            bbox=dict(facecolor=WEISS, edgecolor=WEISS,
                      boxstyle="square,pad=0.25", linewidth=0.8))
ax.view_init(elev=38, azim=-75)
ax.set_zlim(Z_FLOOR, -1.1)
ax.set_xlim(-LIM * 0.85, LIM * 0.85)
ax.set_ylim(-LIM * 0.85, LIM * 0.85)
ax.set_box_aspect((1, 1, 0.55))
fig.subplots_adjust(left=0, right=1, top=1, bottom=0)

save_diagram(fig, 'lagrange_potentialflaeche.png', unterordner='Modelle')
